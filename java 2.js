// تفعيل مكتبة ScrollTrigger أولاً
...
// تحريك الصف الأول يساراً مع السكرول
gsap.to(row1, {
    xPercent: -30,
    ease: "none",
    scrollTrigger: {
...
// تحريك الصف الثاني يميناً مع السكرول
gsap.to(row2, {
    xPercent: 15,
    ease: "none",
    scrollTrigger: {
...
textContainer.innerHTML = originalText.split('').map(char => `<span class="opacity-30 transition-all duration-200">${char}</span>`).join('');
